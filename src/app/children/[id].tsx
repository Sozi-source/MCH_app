...
// FIX: sex field added to EditForm
interface EditForm {
  full_name:        string;
...
export default function ChildDetailScreen() {
  ...
  const { children, growthRecords, fetchGrowthRecords, fetchChildren, selectChild, selectedChildId } = useChildStore();
  ...
  // Edit modal — FIX: sex included in initial state
  ...
  // ── Sync local child state whenever the store updates ─────────────────────
  useEffect(() => {
    const found = children.find(c => c.id === id);
    if (found) {
      setChild(found);
      // FIX: sex synced from store so modal always opens with correct value
      setEditForm({
  ...
  // ── Fetch supporting data once child is resolved ──────────────────────────
  useEffect(() => {
    if (!child) return;
    setLoadingGrowth(true);
    fetchGrowthRecords(child.id).finally(() => setLoadingGrowth(false));
    fetchSecondParent(child);
    fetchMilestoneSummary(child.id);
  }, [child?.id]);
  ...
  const handleEditSave = async () => {
    ...
    const weightVal   = editForm.birth_weight_kg ? parseFloat(editForm.birth_weight_kg) : null;
    const heightVal   = editForm.birth_height_cm ? parseFloat(editForm.birth_height_cm) : null;
    const facilityVal = editForm.health_facility.trim() || null;

    if (editForm.birth_weight_kg && (isNaN(weightVal!) || weightVal! <= 0)) {
      setEditError('Please enter a valid birth weight.');
      return;
    }
    if (editForm.birth_height_cm && (isNaN(heightVal!) || heightVal! <= 0)) {
      setEditError('Please enter a valid birth height.');
      return;
    }

    setEditSaving(true);

    // FIX: all fields including sex are now valid — no TypeScript errors
    const updates: Partial<Child> = {
      full_name:       editForm.full_name.trim(),
      birth_weight_kg: weightVal   ?? undefined,
      birth_height_cm: heightVal   ?? undefined,
      health_facility: facilityVal ?? undefined,
      sex:             editForm.sex,
    };
    ...
    // 1. Update local screen state immediately
    setChild(prev => prev ? { ...prev, ...updates } : prev);

    // 2. Re-fetch into Zustand store so all other screens are in sync
    if (session?.user?.id) {
      fetchChildren(session.user.id);
    }

    setShowEditModal(false);
    Alert.alert('Saved', `${toTitleCase(editForm.full_name.trim())}'s details have been updated.`);
  };
  ...
  const handleShare = async () => {
    ...
    const msg = [
      ...
      `Age: ${getAgeLabel(child.date_of_birth)}`,
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, isFemale ? styles.headerFemale : styles.headerMale]}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerAge}>{getAgeLabel(child.date_of_birth)}</Text>
          {isActive ? (
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.sexToggleRow}>
                  {(['male', 'female'] as const).map(option => {
                    const selected = editForm.sex === option;
                    return (
                      <TouchableOpacity
                        key={option}
                        onPress={() => setEditForm(f => ({ ...f, sex: option }))}
                ...
                {editForm.sex !== child.sex && (
                  <View style={styles.sexWarning}>
                    <Ionicons name="information-circle-outline" size={14} color="#856404" />
                    <Text style={styles.sexWarningText}>
                      Changing sex will affect future growth calculations. Previously recorded data will not be recalculated.
                    </Text>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  sexToggleRow:           { flexDirection: 'row', gap: 10, marginBottom: 8 },
  sexOption:              { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 12, borderRadius: RADIUS.md ?? 10, borderWidth: 1.5, borderColor: COLORS.border ?? '#E8ECF0', backgroundColor: COLORS.surface ?? '#F7F8FA' },
  sexOptionSelected:      { borderColor: COLORS.primary, backgroundColor: COLORS.primaryLight ?? '#EDF5FF' },
  sexOptionText:          { fontSize: 14, fontWeight: '600', color: COLORS.textMuted },
  sexOptionTextSelected:  { color: COLORS.primary },
  sexWarning:             { flexDirection: 'row', alignItems: 'flex-start', gap: 6, backgroundColor: '#FFF8E1', borderRadius: RADIUS.md ?? 10, padding: 10, marginBottom: 8 },
  sexWarningText:         { fontSize: 12, color: '#856404', flex: 1, lineHeight: 18 },
});